%% Figure3d Simulation %%
% This script computes the data for **Figure 3d** from the paper:
% "Functional stability and recurrent STDP in rhythmogenesis"
%
% Description:

%       - Computes analytically the border of which above is the CR region. It is in
%       tau_E,m as a function of alpha for different tau_I,m
...
tau_mI_i=2; % inital value of tau_mI (m is minus)
dtau_mI=0.5;
tau_mI_f=5; % final value
tau_mI_vec=tau_mI_i:dtau_mI:tau_mI_f; % its vector of values
q=1; % index for different tau_mI

alpha_cr=nan(length(tau_mI_vec),length(alpha_vec)); % critical alpha (empty) 
tau_mE_cr=nan(length(tau_mI_vec),length(alpha_vec)); % critical tau_mE (empty)
for tau_mI=tau_mI_i:dtau_mI:tau_mI_f % run on different values of tau_mI
...
%%%%%% Extras (not required for the figure) %%%%%%
