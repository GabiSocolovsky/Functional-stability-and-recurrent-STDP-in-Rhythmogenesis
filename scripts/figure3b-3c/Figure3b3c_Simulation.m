%% Figure 3b-3c Simulation %%
% This script generates **Figures 3b-3c** from the paper:
% "Functional stability and recurrent STDP in rhythmogenesis"
%
% Description:

%       - Computes the order parameter dynamics of Jei and Jie in the limit
%       of slow learning (figure 3b)
%       - Computes the order parameter dynamics of Jei and Jie for in
%       a continuous simulation (pink traces in figure 3b)
%       - Computes and shows the dynamics of the order parameters of m_E
%       nad m_I in the continuous case.

% Dependencies:

%       - Correlations_2D_full_diff.m - computes the correlations
%       - Two_populations_full_rate_model_history.m - computes the firing
%       rates dynamics
...
tf=200; % final time
D=0.4; % delay in msec
Jii=0.4; % Jii order parameter
Jee=0.6; % Jee order parameter
Jhat=(Jee+Jii)/2;
...
alpha_arr=[0.95:0.01:0.98 0.98]; % relative depression (different values) %
Jei_arr=[0.1 0.2 0.5 0.1 0.75]; % different starting points for Jei
Jie_arr=[10 9 10 5 6.5]; % different starting points for Jie
...
lambda_e=4; % excitatory synapses learning rate
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%% Temporal Kernels %%%%%%%%%%%%%
Jei=0.1; % arbitray values for computing in next lines the temporal kernels
Jie=10; % arbitray values for computing in next lines the temporal kernels
[Corr_ie_final,Corr_ei_final,~,~,~,~,~,~,Delta_extended]=proj.common.Correlations_2D_full_diff(Jee,Jei,Jie,Jii,dt,tf); % computing Delta_extended
K_pE=1/tau_pE*exp(-Delta_extended*H_E/tau_pE).*heaviside(H_E*Delta_extended); % temporal kernels
...
% Define neat yellow and orange
orange = [1.0, 0.5, 0.0];  % Rich orange
green  = [0.0, 0.7, 0.3];  % Natural green

plot(JbarD^2./(0:0.1:20),0:0.1:20,'Color','Black','LineWidth',3) % plot bif. line
hold on
plot((1+Jii)*ones(1,length(0:20)),0:20,'Color','Black','LineWidth',3) % plot line between FP and OI
xlabel('$J_{EI}$','interpreter','latex','FontSize',18)
ylabel('$J_{IE}$','interpreter','latex','FontSize',18)


for p=1:length(alpha_arr) % runs dynamics for different alpha
    ...
    t=1:10000; % time for simulation per alpha
    dtlearn=1; % timebin for STDP dynamics
    
    J_ie_final=Jiemax*(1-((1-K_Ibar)*(K_Ibar*K_Emtilpsi-K_Itilpsi)/(K_Ibar*K_Eptilpsi-K_Itilpsi))^(1/mu)); % the place where CR settles    
    plot(JbarD^2/J_ie_final,J_ie_final,'*','Color',[0 0 1],'MarkerSize',14,'LineWidth',1.6) % plot this point

    
    
    % Change colour per alpha %
    ...
    Jeimeanarr(1)=Jei; % Jei strating point of dynamics 
    Jiemeanarr(1)=Jie; % Jie strating point of dynamics 
    
    
    
    for i=2:length(t) % start dyanmics
        ...
        if Jei*Jie<JbarD^2 % in the FP regime solve exactly
            me=(1+Jii-Jei)/((1-Jee)*(1+Jii)+Jei*Jie); % m_E
            mi=(1+Jie-Jee)/((1-Jee)*(1+Jii)+Jei*Jie); % m_I
        ...
        Jei=Jei+dtlearn*J_ei_dot; % update
        Jie=Jie+dtlearn*J_ie_dot; % update
        
        Jeimeanarr(i)=Jei; % update
        Jiemeanarr(i)=Jie; % update
        
        
        plot(Jei,Jie,'.','Color',color_k ,'HandleVisibility', 'off') % plot the dynamics in every loop
        
        if ((Jeimeanarr(i)-Jeimeanarr(i-1))^2+(Jiemeanarr(i)-Jiemeanarr(i-1))^2)^0.5<0.0001 % condition for terminating STDP dynamics
            break;
        end
    end
end
...
saveas(fig,fullPath) % save figure in path
%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%% Add legend %%%%%%
figure(1)
