%% Figure2b Simulation %%
% This script generates **Figure 2b** from the paper:
% "Functional stability and recurrent STDP in rhythmogenesis"
%
% Description:
      
%       Computes and plots the phase diagram of the neural network in the
%       [J_bar, J_EE] plane for different J_II values, showing the oscillation frequency as a contour plot.
%       Uses parfor loop for computing the frequency contour.


% Dependencies:

%       - FrequencyContourJbarJee.m 
%
% Authors: Gabi Socolovsky & Maoz Shamir
% Date: 2025-09-29
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%% Parameters %%%%%%%%%%%%%%%
T=1; % Time scale for the firing rate dynamics - Tau_m=1
Tunits  = 5e-3; % Time unit = Tau_m = 5 ms
dt=0.01; % time bin
tf=200; % final time
D=0.4; % delay

% Arrays of synaptic connections %
Jii_init=0; 
dJii=0.2;
Jii_final=0.8;
Jii_arr=Jii_init:dJii:Jii_final;
Jee_init=0;
dJee=0.05;
Jee_final=5;
Jee_arr=(Jee_init:dJee:Jee_final)+0.001; 
JbarD_arr=nan(length(Jii_arr),length(Jee_arr));
wD_arr=nan(length(Jii_arr),length(Jee_arr));

%%%%%%%%%%% Compute phase diagram and the freuqnecy contour %%%%%%%%%%% 
for i=1:length(Jii_arr)
    Jii=Jii_arr(i);
    for j=1:length(Jee_arr) % for a given Jii, compute phase diagram
        Jee=Jee_arr(j);
        syms wD JbarD % frequency and Jbar on bif. line
        if Jee>=2+Jii % outside of the FP-R bif. line
            break        
        elseif Jee>Jii 
            range=[0.1 5 ;0.2 pi/(2*D)]; % value range for solving w and Jbar on the bif. line
            Y=vpasolve([(JbarD^2-Jee*Jii)^0.5==1/cos(wD*D-acos((Jee-Jii)/(2*(JbarD^2-Jee*Jii)^0.5))), T*wD==-tan(wD*D-acos((Jee-Jii)/(2*(JbarD^2-Jee*Jii)^0.5)))], [JbarD,wD],range); % the frequency on the bifurcation line
            JbarD_arr(i,j)=double(Y.JbarD); % JbarD
            wD_arr(i,j)=double(Y.wD); % wD
        elseif Jee<Jii 
            range=[0.1 5 ;0.2 pi/D];  % value range for solving w and Jbar on the bif. line  
            Y=vpasolve([(JbarD^2-Jee*Jii)^0.5==1/cos(wD*D-acos((Jee-Jii)/(2*(JbarD^2-Jee*Jii)^0.5))), T*wD==-tan(wD*D-acos((Jee-Jii)/(2*(JbarD^2-Jee*Jii)^0.5)))], [JbarD,wD],range); % the frequency on the bifurcation line
            JbarD_arr(i,j)=double(Y.JbarD); % JbarD
            wD_arr(i,j)=double(Y.wD); % wD
        elseif Jee==Jii
            range=[0.1 pi/(2*D)-0.01]; % value range for solving w and Jbar on the bif. line
            Y=vpasolve(T*wD==cot(wD*D),wD,range); % the frequency on the bifurcation line            
            JbarD_arr(i,j)=(1+(double(Y)*T)^2)^0.5; % JbarD
            wD_arr(i,j)=double(Y); % wD
        end
    end
    JbarD_arr(i,j)=1+Jii; % last value of JbarD (tip of the bif. line)

    %%%% Frequency Contour for Jii=0.8 %%%%
    if Jii==0.8 
        JbarD_arrlast=JbarD_arr(end,:);
        Jbar_arrlast=0:0.002:4;
        f=nan(length(Jbar_arrlast),length(Jee_arr)); % frequency array (empty)
        parfor k=1:length(Jee_arr)
            Jee=Jee_arr(k);
            JbarD=JbarD_arrlast(k);
            if Jee<=2+Jii
                f(:,k)=proj.common.FrequencyContourJbarJee(JbarD,Jbar_arrlast,Jee,Jii,dt,tf);
            else
                f(:,k)=proj.common.FrequencyContourJbarJee(nan,Jbar_arrlast,Jee,Jii,dt,tf);
            end
        end
            f=f/(Tunits); % frequency grid in units of Hz
            [JBAR_arrlast,JEE_arr]=meshgrid(Jbar_arrlast,Jee_arr);
        [C,h]=contourf(JBAR_arrlast,JEE_arr,f',100, 'HandleVisibility', 'off'); % plot contour of frequency
        set(h,'LineColor','none')
        grid on
    end
    plot(JbarD_arr(i,:),Jee_arr,'Color','black','LineWidth',3,'Color',[(i-1)/(length(Jii_arr)+1) (i-1)/(length(Jii_arr)+1) (i-1)/(length(Jii_arr)+1)])
    hold on
    plot(0:0.01:(1+Jii),1+(0:0.01:(1+Jii)).^2/(1+Jii),'LineWidth',3,'Color',[(i-1)/(length(Jii_arr)+1) (i-1)/(length(Jii_arr)+1) (i-1)/(length(Jii_arr)+1)], 'HandleVisibility', 'off')
    plot(((1+Jii):0.1:4),2*((1+Jii):0.1:4)-Jii,'LineWidth',3,'Color',[(i-1)/(length(Jii_arr)+1) (i-1)/(length(Jii_arr)+1) (i-1)/(length(Jii_arr)+1)], 'HandleVisibility', 'off')
end
plot(0:0.5:4,ones(1,length(0:0.5:4)),'--','Color','red', 'HandleVisibility', 'off')
lgd=legend({'$0$','$0.2$','$ 0.4$','$0.6$','$0.8$'},'Interpreter','latex');
legendJiiTitle=sprintf('$J_{II}$','Interpreter','latex');
lgd.Title.String = legendJiiTitle;
xlabel('$\bar{J}$','interpreter','latex','FontSize',18)
ylabel('$J_{EE}$','interpreter','latex','FontSize',18)
text(0.8,0.4,'$\mathrm{FP}$','interpreter','latex','FontSize',24)
text(3,3,'$\mathrm{R}$','interpreter','latex','FontSize',24)
text(0.6,3.5,'$\mathrm{D}$','interpreter','latex','FontSize',24)
xlim([0 4])
ylim([0 5])
set(gca,'FontSize',14)
set(gca,'TickLabelInterpreter','latex')